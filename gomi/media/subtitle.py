...
from ..fs import tstFileEncoding


ASS_INLINE_FONTNAME_BASE_PATTERN = re.compile(r"\\fn([^\\}]+)")
ASS_INLINE_STYLENAME_BASE_PATTERN = re.compile(r"\\r([^\\}]+)")
...
def toAssFileObjs(
    paths: list[Path], encoding: str | list[str] = "utf-8-sig", test: bool = False
) -> list[AssFile | None]:
